...
The operator reconcile contract is: `ListActiveRuns` → for each active run, poll `SubscribeRunEvents(since_seq)` → advance its per-run cursor to `last_seq` (persisting `next_since_seq` on truncated pages) → apply run and adapter lifecycle to its own reconcile state.
